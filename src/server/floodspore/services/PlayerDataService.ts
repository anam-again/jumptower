...
export class PlayerDataService implements OnInit {
	...
	public changeCoinBalance(player: Player | undefined, amount: number) {
		if (!player) return;
		const data = this.getProfile(player);
		if (data) {
			store.changeBalance(tostring(player.UserId), "Coins", amount);
		}
	}

	public canAfford(player: Player | undefined, amount: number): boolean {
		if (!player) return false;
		const data = this.getProfile(player);
		return data ? data.Data.balance.Coins >= amount : false;
	}
}
